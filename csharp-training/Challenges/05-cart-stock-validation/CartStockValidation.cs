...
public class CartStockValidation
{
    ...
    {
        PrintResult("10 Pencils", AddToCart(102, 10));
        PrintResult("5 Pencils",  AddToCart(102, 10));
    }

    private static CustomerCartStatus AddToCart(int productId, int quantity)
    {
    ...
    private static void GetCartTotal()
    {
        foreach (var (key, value) in DictionaryOfSelectedProducts)
        {
            Console.WriteLine($"Product: {key}: Rs {value.Quantity * value.UnitPrice}");
        }
    }
